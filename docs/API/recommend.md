...
1. 用户在 Recommend.vue 中设置筛选条件（人均、地区、菜系、口味、距离等）
2. 点击“随机推荐”按钮
3. 前端向后端发送请求，从候选餐馆中随机返回 **0–1 条记录**
...
- **口味**：`flavors`
- **评分下限**：`min_rating`
- **距离限制**：`max_distance_km + lat/lng`

> 若未提供经纬度，将返回一个 **伪距离（假数据）**，用于前端展示。
