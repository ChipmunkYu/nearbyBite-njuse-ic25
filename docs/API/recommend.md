...
GET	/api/recommend/restaurants	获取一条随机推荐餐馆记录
...
400	    参数格式错误（例如 price_min > price_max）
500  	服务器内部错误
...
const fetchRandom = async () => {
  ...
  results.value = res.data || res // 视后端封装而定
  showModal.value = results.value.length > 0
}

// 弹窗打开时写入 History（与 History 模块文档对齐）
